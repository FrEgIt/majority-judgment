    ...
    return [
            {
                ...
                "balou": random.randint(1, 6),
                "chuck-norris": random.randint(0, 4),
                "elsa": random.randint(1, 3),
                "gandalf": random.randint(3, 5),
                "beyonce": random.randint(2, 4)
...
def sort_candidates_by(mention):
    ## buble sort here we go!
    unsorted = [(key, (mention["mention"], mention["score"])) for key, mention in mention.items()]
    swapped = True
    while swapped:
        swapped = False
        for j in range(0, len(unsorted) - 1):
            ## but we need REVERSE buble sort ;-)
